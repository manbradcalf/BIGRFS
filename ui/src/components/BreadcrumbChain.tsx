...
/**
 * Parses relationship text like "Player - HAD -> PlayerGame" or "Movie <- ACTED_IN - Actor" into separate parts
 */
const parseRelationshipChunk = (text: string): RelationshipPart[] => {
  // Match pattern: "EntityA - RELATIONSHIP -> EntityB" (outgoing relationship)
  const outgoingMatch = text.match(/^(.+?)\s*-\s*(.+?)\s*->\s*(.+)$/);

  if (outgoingMatch) {
    const [, fromEntity, relationship, toEntity] = outgoingMatch;
    return [
      {
        type: "entity",
  ...
  // Match pattern: "EntityA <- RELATIONSHIP - EntityB" (incoming relationship)
  const incomingMatch = text.match(/^(.+?)\s*<-\s*(.+?)\s*-\s*(.+)$/);

  if (incomingMatch) {
    const [, toEntity, relationship, fromEntity] = incomingMatch;
    return [
      {
        type: "entity",
        text: toEntity.trim(),
